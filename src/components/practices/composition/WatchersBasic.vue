...
watch(currentCity, (newValue, oldValue) => {
  ...
  console.log(`[서버 요청 완료] 기상청 서버에서 ${newValue}의 날씨 API를 다시 불러옵니다,`)
})
</script>
...
<template>
  <div class="practice-section">
    ...
    <button @click="currentCity = '서울'">서울 선택</button>&nbsp;<button
      @click="currentCity = '수원'"
    >
      수원 선택</button
    >&nbsp;
    ...
    <div class="monitor">
      <h3>모니터링 시스템</h3>
      <p>{{ logMessage }}</p>
      <small style="color: gray">f12 콘솔창 확인</small>
    </div>
  </div>
</template>
